def strategist_agent(requirement,evidence):
    return {"agent":"Strategist","positioning":"Use only traceable NovaTech evidence and answer the requirement directly.","sources":[e.get("source") for e in evidence[:3]]}
_NEGATION_MARKERS = (
...
def compliance_agent(requirement,evidence):
    ...
    # evidence that mentions a term only to explicitly deny it (e.g. "NovaTech
    # did NOT claim FedRAMP authorization"). Check each term against the
    # specific chunk it appears in so a denial reads as MISSING, not PASS.
    for term in exact_terms:
        ...
        if negated and not supported:
            return {"agent":"Compliance","assessment":"MISSING","reason":f"Evidence explicitly states NovaTech does not offer this ({term})."}
        if not supported:
