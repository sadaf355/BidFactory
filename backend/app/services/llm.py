...
def generate_response(requirement: str, evidence: list[dict], status_hint: str | None = None) -> dict:
    settings = get_settings()
    context = "\n\n".join(f"[{e.get('source', 'unknown')}] {e.get('text', '')}" for e in evidence)
    client = _client()

    if client:
        prompt = f'''You are Bid Factory, an evidence-grounded proposal assistant for NovaTech Solutions. Answer the RFP requirement using ONLY the evidence below. Never upgrade a partial capability to a full capability. If evidence is insufficient, say so. Return JSON with keys: status (PASS/PARTIAL/MISSING), confidence (0-1), answer, rationale.
...
{context or 'NO EVIDENCE FOUND'}'''
    ...
    if not evidence:
        return {
            "status": "MISSING",
            "confidence": 0.98,
            "answer": "NovaTech could not find supporting evidence for this requirement in the current knowledge base.",
    ...
    answer = f"NovaTech's documented evidence in {top.get('source', 'the knowledge base')} indicates: {top.get('text', '')[:700]}"
    return {
        "status": status,
