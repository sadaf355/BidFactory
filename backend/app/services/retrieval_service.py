"""
Evidence retrieval against the vendor's own knowledge base (whatever
documents have been uploaded via the Knowledge Base page).
        ...
        for p in KB.rglob("*"):
            if p.suffix.lower() in (".md", ".txt", ".json"):
                try:
                    ...
                    if len(t) <= 3000:
                        if t.strip():
                            out.append({"source": p.relative_to(KB).as_posix(), "text": t, "metadata": {"category": p.parent.name}})
                    else:
                        for i in range(0, len(t), 1800):
                            c = t[i:i + 2500]
                            if c.strip():
                                out.append({"source": p.relative_to(KB).as_posix(), "text": c, "metadata": {"category": p.parent.name}})
                except Exception:
                    pass
